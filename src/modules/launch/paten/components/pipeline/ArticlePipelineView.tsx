...
export const ArticlePipelineView: React.FC<ArticlePipelineViewProps> = ({
  ...
  // Filter logic. Saat unit aktif = "Semua Unit Bisnis" jangan menyaring per
  // unit — sebelumnya baris ini menyembunyikan seluruh artikel secara default.
  const filteredArticles = articles.filter((a) => {
    if (!isAllBusinessUnits(activeBusinessUnit) && a.businessUnit !== activeBusinessUnit) return false;
    if (searchQuery) {
  ...
  return (
    <div className="space-y-5 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 bg-white p-4 rounded-2xl border border-slate-200 shadow-2xs">
        ...
        {/* View Switcher & Action — di mobile switcher jadi ikon-saja & tombol
            memenuhi lebar agar tidak berdesakan. */}
        <div className="flex items-center gap-2 w-full md:w-auto">
          <div className="flex items-center p-1 bg-slate-100 rounded-xl text-xs font-bold border border-slate-200 shrink-0">
            {([
              { mode: 'grid' as const, icon: Grid, label: 'Grid' },
              { mode: 'list' as const, icon: List, label: 'List' },
              { mode: 'kanban' as const, icon: KanbanIcon, label: 'Kanban' },
          ...
          {canCreate && onOpenQuickCreate && (
            <button
              ...
              className="paten-primary-action flex flex-1 md:flex-none items-center justify-center gap-1.5 px-4 py-2 text-white text-xs font-bold transition-all shrink-0"
            >
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            <select
              ...
              className="flex-1 sm:flex-none px-3 py-1.5 rounded-xl border border-slate-200 bg-white text-xs font-semibold text-slate-800 focus:outline-none focus:border-[#087E79]"
            >
