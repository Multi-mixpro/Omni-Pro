import { BookOpen, CircleDollarSign, ExternalLink, Factory, FileImage, Palette, Phone, Ruler } from 'lucide-react';
...
export function BriefSnapshot({ references, materials, colorways, sizeCharts, hpp }: Pick<ProjectWorkspace, 'references' | 'materials' | 'colorways' | 'sizeCharts' | 'hpp'>) {
  ...
  return (
    <section className="brief-snapshot">
      ...
      {(imageReferences.length > 0 || learningLinks.length > 0) && <div className="brief-snapshot-block"><div className="brief-snapshot-title"><FileImage size={17} /><b>Referensi visual & sumber belajar</b></div>{imageReferences.length > 0 && <div className="snapshot-gallery">{imageReferences.map(item => <a href={item.source_url || item.image_url || '#'} target="_blank" rel="noreferrer" key={item.id}><img src={item.image_url || ''} alt={item.title} /><span>{item.is_primary && <small>Cover</small>}<b>{item.title}</b></span></a>)}</div>}{learningLinks.length > 0 && <div className="snapshot-links">{learningLinks.map(item => <a href={item.source_url || '#'} target="_blank" rel="noreferrer" key={item.id}><span><small>{item.reference_type}</small><b>{item.title}</b><p>{item.insight || 'Buka sumber untuk dipelajari tim.'}</p></span><ExternalLink size={17} /></a>)}</div>}</div>}
